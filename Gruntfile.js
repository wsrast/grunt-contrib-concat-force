...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		concat: {
			force_option: {
				...
				files: {
					'tmp/readonly': ['test/fixtures/file1', 'test/fixtures/file2']
				}
			},
			default_options: {
				files: {
	 ...
	 * then copy the read-only file to the /tmp folder,
	 * then chmod the permissons on 'tmp/readonly' to be read-only
	 * then run this plugin's task(s),
	 * then test the result.
	 */
	grunt.registerTask('test', ['jshint', 'clean', 'copy', 'chmod', 'concat', 'nodeunit']);
